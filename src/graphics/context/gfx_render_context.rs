...
pub struct GfxRenderContext<B: Backend> where B::Device: Send + Sync {
    instance: B::Instance,
...
impl<B: Backend> GfxRenderContext<B> {
    pub fn new(name: &str, window: Rc<winit::window::Window>) -> Self {
        ...
        let (device, mut queue_group) = {
            ...
            let mut families = &adapter
                .queue_families;


            let graphics_family = families.iter().find(|family| {
            ...
            let mut gpu = unsafe {
                ...
                adapter
                    .physical_device
                    .open(&[(graphics_family, &[1.0; 1]), (compute_family, &[1.0; 1])], gfx_hal::Features::empty())
